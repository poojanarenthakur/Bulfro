...
const { project_validation } = require('../validation/validation');
...
router.post('/add_projects', [checkadmin], project_validation(), (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        const query = `INSERT INTO projects(project_name, org_id, org_location_id, current_status)
     VALUES (?, ?, ?, ?)`;
        connection.query(query, [project_name, org_id, org_location_id, current_status], (error, result) => {
            if (error) {
                const resp = {
                    success: false,
                    message: ' error ' + error,
...
router.put('/update_projects/:id', [checkadmin], (req, res) => {
    const { id } = req.params;
...
router.delete('/delete_projects/:id', [checkadmin], (req, res) => {
    const { id } = req.params; //got id from URL parameter
    const sqlQuery = `DELETE FROM projects WHERE id = ?`; //created sql query
...
router.get('/get_all_projects', [checkadmin], (req, res) => {
    const sqlQuery = `SELECT * FROM projects`; //created sql query
...
//get admin by id API
router.get('/get_projects/:id', [checkadmin], (req, res) => {
    const { id } = req.params; //got id from URL parameter
    const sqlQuery = `SELECT * FROM projects WHERE id = ?`; //created sql query
    connection.query(sqlQuery, [id], (err, result) => {
        if (err) {
            const resp = {
                success: false,
