...
const { admin_validation } = require('../validation/validation');
...
//add admin
router.post('/add_admin', [checkadmin], admin_validation(), (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        const { first_name,
            ...
            address,
            is_active } = req.body;
        const query = `INSERT INTO admin (first_name, last_name, email, username, password, mobile, address, is_active)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
        connection.query(query, [first_name, last_name, email, username, password, mobile, address, is_active], (error, result) => {
            if (error) {
                const resp = {
                    success: false,
                    message: ' error ' + error,
...
router.get('/get_all_admin', [checkadmin], (req, res) => {
    const sqlQuery = `SELECT * FROM admin`; //created sql query
...
router.get('/get_admin/:id', [checkadmin], (req, res) => {
    const { id } = req.params; //got id from URL parameter
    const sqlQuery = `SELECT * FROM admin WHERE id = ?`; //created sql query
    connection.query(sqlQuery, [id], (err, result) => {
        if (err) {
            const resp = {
                success: false,
